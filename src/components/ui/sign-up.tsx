  ...
  const canvasRef = useCallback((node: HTMLCanvasElement) => {
    if (node !== null) {
      ...
      if (instanceRef.current) {
        if (instanceRef.current.reset) instanceRef.current.reset()
...
const GradientBackground = () => (
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full">
            <defs>
                <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'var(--primary)', stopOpacity:0.8}} /><stop offset="100%" style={{stopColor: 'var(--secondary)', stopOpacity:0.6}} /></linearGradient>
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'var(--accent)', stopOpacity:0.9}} /><stop offset="50%" style={{stopColor: 'var(--secondary)', stopOpacity:0.7}} /><stop offset="100%" style={{stopColor: 'var(--primary)', stopOpacity:0.6}} /></linearGradient>
                <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%"><stop offset="0%" style={{stopColor: 'var(--destructive)', stopOpacity:0.8}} /><stop offset="100%" style={{stopColor: 'var(--primary)', stopOpacity:0.4}} /></radialGradient>
            ...
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                ...
                <ellipse cx="50" cy="150" rx="180" ry="120" fill="var(--accent)" filter="url(#rev_blur2)" opacity="0.8"/>
            </g>
        </svg>
    </>
);
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Guardian Portal", onSuccess }: AuthComponentProps) => {
  const auth = useAuth();
